...
def _render_page_to_png(page: fitz.Page, zoom: float = 2.0) -> bytes:
    ...
    *,
    ...
    for image_index, image_info in enumerate(page.get_images(full=True)):
        ...
        storage_key = storage_service.build_generated_key(
            ...
            str(document.id),
            "images",
            f"page_{page_number}_image_{image_index}_{image_id}.{image_ext}",
        )
...
def ingest_pdf_document(db: Session, document_id: uuid.UUID) -> IngestResult:
    ...
    with fitz.open(pdf_path) as pdf_document:
        ...
        for page_index in range(pdf_document.page_count):
            ...
            page_png = _render_page_to_png(page)
            ...
            image_count += _extract_embedded_images(
                db=db,
                document=document,
                document_page=document_page,
